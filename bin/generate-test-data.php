...
try {
	for ( $i = 0; $i < $number_of_people; $i++ ) {
		...
		// Set meta data.
		update_post_meta( $person_id, 'ph_peeps_first_name', $first_name );
		update_post_meta( $person_id, 'ph_peeps_middle_name', $middle_name );
		...
		update_post_meta( $person_id, 'ph_peeps_social_links', $social_links );

		// Add featured image.
		$seed      = rawurlencode( $first_name . $last_name );
		...
		// Download avatar via WordPress HTTP API.
		...
		if ( empty( $image_data ) ) {
			WP_CLI::warning( sprintf( 'Failed to download avatar for person %d.', $i + 1 ) );
			continue;
		}

		$filename = "person-{$person_id}-" . time() . '.svg';
		$file     = $upload_dir['path'] . '/' . $filename;

		// Make sure the image library and WP_Filesystem are loaded.
		...
		$attach_id = wp_insert_attachment( $attachment, $file, $person_id );
		...
		$result = set_post_thumbnail( $person_id, $attach_id );
		if ( ! $result ) {
			WP_CLI::warning(
				sprintf(
